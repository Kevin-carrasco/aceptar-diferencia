...
# segunda estimacion (mrb)

```{r}
load("IPO/input/data/proc/data_comuna.RData")
data_comunas <- data_comuna %>% dplyr::select(cod_com_alu=p10comuna, escolaridad, etnia, inmigrante, prop_etnia, prop_inm)
...
```{r}
screenreg(list(reg1.1, reg2.1, reg3.1, reg4.1, reg5.1))
```
...
## Prueba con cod_com_alu

```{r}
reg0.11 <- lmer(est_acc_div ~ 1 + (1 | cod_com_alu), data=base)
reg1.11 <- lmer(est_acc_div ~ educacion_rec + (1 | cod_com_alu), data=base)
reg2.11 <- lmer(est_acc_div ~ libros_rec + (1 | cod_com_alu), data=base)
reg3.11 <- lmer(est_acc_div ~ apod_acc_div + (1 | cod_com_alu), data=base)
reg4.11 <- lmer(est_acc_div ~ civic + (1 | cod_com_alu), data=base)
reg5.11 <- lmer(est_acc_div ~ ap_dis + (1 | cod_com_alu), data=base)
reg6.11 <- lmer(est_acc_div ~ mean_apdis + (1 | cod_com_alu), data=base)
reg7.11 <- lmer(est_acc_div ~ etnia2 + (1 | cod_com_alu), data=base)
reg8.11 <- lmer(est_acc_div ~ inmigrante2 + (1 | cod_com_alu), data=base)
reg9.11 <- lmer(est_acc_div ~ escolaridad + (1 | cod_com_alu), data=base)
reg10.11 <- lmer(est_acc_div ~ educacion_rec + libros_rec + apod_acc_div + civic + ap_dis + mean_apdis + etnia2 + inmigrante2 + escolaridad + (1 | cod_com_alu), data=base)
```

```{r}
screenreg(list(reg1.11, reg2.11, reg3.11, reg4.11, reg5.11, reg6.11, reg7.11, reg8.11, reg9.11, reg10.11))
```
...
reg14 <- lmer(est_acc_div ~ educacion_rec + libros_rec + apod_acc_div*etnia2 + civic + ap_dis + mean_apdis + etnia2 + inmigrante2 + escolaridad + (1 | mrbd), data=base)
...
```{r}
screenreg(list(reg11, reg12, reg13, reg14, reg15))
screenreg(list(reg1.1, reg2.1, reg3.1, reg4.1, reg5.1, reg12, reg13, reg15))
...
```{r}
plot_model(reg13, type="int", mdrt.values = "meansd")
```
