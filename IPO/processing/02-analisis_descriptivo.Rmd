...
```{r}
pacman::p_load(dplyr, ggplot2, sjPlot, summarytools, corrplot, ltm, ggpubr, ggmosaic)
...
# dicotomizar var dep
data = data %>% rowwise() %>%  
  mutate(est_act_dic = case_when(est_acc_div==7~"Acepta todos",
                                 est_acc_div<=6~"Rechaza uno o más"
                                              ))
...
data %>% filter(educacion_dic!="Ns/Nr") %>% 
  ggplot() +
  geom_mosaic(aes(product(est_act_dic, educacion_dic), fill= est_act_dic)) + labs(y = "Actitudes estudiantes", x = "Educación Apoderados")
```


```{r}
# dicotomizar actitudes apoderados
data = data %>% rowwise() %>%  
  mutate(apod_act_dic = case_when(apod_acc_div==7~"Acepta todos",
                                 apod_acc_div<=6~"Rechaza uno o más"
                                              ))

data %>% 
  ggplot() +
  geom_mosaic(aes(product(est_act_dic, apod_act_dic), fill= est_act_dic)) + labs(y = "Actitudes estudiantes", x = "Actitudes Apoderados")
```
...
data %>% filter(civic_dic!=is.na(civic_dic)) %>% 
  ggplot() +
  geom_mosaic(aes(product(est_act_dic, civic_dic), fill= est_act_dic)) + labs(y = "Actitudes estudiantes", x = "Conocimiento cívico")
```
...
# dicotomizar actitudes apoderados
data = data %>% rowwise() %>%  
  mutate(apdis_dic = case_when(apdis_fa>=0.1450~"Alto",
                               apdis_fa<0.1450~"Bajo"
                                              ))

data %>% filter(apdis_dic!=is.na(apdis_dic)) %>% 
  ggplot() +
  geom_mosaic(aes(product(est_act_dic, apdis_dic), fill= est_act_dic)) + labs(y = "Actitudes estudiantes", x = "Apertura a la discusión")


ggarrange(educacion_plot, actitudes_plot, civic_plot, apdis_plot)
```
